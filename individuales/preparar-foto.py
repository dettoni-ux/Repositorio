...
"""Deja una foto lista para usarse de fondo del individual.
    ...
    python3 individuales/preparar-foto.py entrada.jpg playa --zoom 1.15
...
Con --zoom acerca la imagen: 1.15 es un 15% más cerca, recortando desde el
centro. Ojo que acercar también baja los ppp efectivos.
...
def recortar(foto: Image.Image, zoom: float = 1.0) -> Image.Image:
    """Escala hasta llenar la hoja y recorta lo que sobra, centrado."""
    escala = max(DESTINO_ANCHO / foto.width, DESTINO_ALTO / foto.height) * zoom
...
def main() -> int:
    ...
    ppp_real = min(foto.width / HOJA_ANCHO_MM, foto.height / HOJA_ALTO_MM) * 25.4 / zoom
    salida = expandir(foto) if modo_expandir else recortar(foto, zoom)
    ...
    detalle = 'expandida' if modo_expandir else 'recortada'
    if zoom != 1.0:
        detalle += f', {round((zoom - 1) * 100)}% más cerca'
    print(f'→ {destino.relative_to(AQUI.parent)}  {DESTINO_ANCHO} x {DESTINO_ALTO} px ({detalle})')
    if ppp_real < 240:
