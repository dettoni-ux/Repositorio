...
/* Los colores de la web real: morado y amarillo. El rosado del logo es solo
   del logo; la plataforma no es rosada. */
const MORADO = '#5B2E7E';
const MORADO_CLARO = '#EFE7F5';
const MORADO_SUAVE = '#F7F3FA';
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .barra{background:#fff;padding:14px 28px 20px;display:flex;align-items:center;gap:18px;
         border-bottom:2px solid #EDE6F2}
  .barra img{height:62px}
  .flecha,.lupa{color:${TINTA};font-size:34px;line-height:1;opacity:.75}
  .buscar{margin:26px 28px 0;background:${MORADO_SUAVE};border-radius:999px;
          padding:20px 26px;color:#7E7286;font-size:25px;display:flex;gap:14px;align-items:center}
  h2{margin:40px 28px 18px;font-size:34px;font-weight:800;letter-spacing:-.5px}
  .tarjetas{display:flex;gap:18px;margin:0 28px}
  .t{flex:1;background:${MORADO_SUAVE};border-radius:26px;padding:26px 20px 24px;text-align:center}
  .t.act{background:${MORADO_CLARO};outline:3px solid ${MORADO}}
  ...
  .fila{margin:0 28px 14px;background:${MORADO_SUAVE};border-radius:22px;
        padding:22px 24px;display:flex;align-items:center;gap:20px}
  .fila .pt{width:62px;height:62px;border-radius:50%;background:${MORADO};
            display:grid;place-items:center;flex:0 0 auto}
  .fila .pt svg{width:34px;height:34px}
  .fila.destaca{background:#FFF7DE;border:2px solid ${AMARILLO}}
  .fila.destaca .pt{background:${AMARILLO}}
  .fila.destaca .pt svg{stroke:${TINTA}}
  .fila .tx b{display:block;font-size:27px;font-weight:700}
  .fila .tx s{display:block;font-size:22px;color:#7E7286;text-decoration:none;margin-top:3px}
  .fila .fl{margin-left:auto;color:#B3A7BD;font-size:30px}
  .cta{margin:36px 28px 0;background:${MORADO};color:#fff;border-radius:999px;
       text-align:center;padding:26px;font-size:30px;font-weight:800}
  .tabs{position:absolute;bottom:0;left:0;right:0;border-top:2px solid #EDE6F2;
        display:flex;padding:20px 0 34px;background:#fff}
  .tab{flex:1;text-align:center;font-size:20px;color:#A99DB3}
  .tab .i{font-size:34px;display:block;margin-bottom:6px}
  .tab.on{color:${MORADO};font-weight:700}
...
<h2>Servicios</h2>
<div class="tarjetas">
  <div class="t act"><div class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="${MORADO}" stroke-width="1.7" stroke-linecap="round"><rect x="3" y="5" width="18" height="16" rx="3"/><path d="M3 10h18M8 3v4M16 3v4"/><circle cx="8.5" cy="14.5" r="1.3" fill="${MORADO}" stroke="none"/><circle cx="12" cy="14.5" r="1.3" fill="${MORADO}" stroke="none"/></svg></div><div class="n">Agendar<br>hora</div></div>
  <div class="t"><div class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="#9B85AC" stroke-width="1.7" stroke-linecap="round"><rect x="4" y="3" width="16" height="18" rx="3"/><path d="M9 2.5h6v3H9zM8 11h8M8 15h5"/></svg></div><div class="n">Ficha del<br>paciente</div></div>
</div>
